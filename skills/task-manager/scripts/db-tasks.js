...
function addTask(text, status = 'backlog') {
  ...
  const maxTaskNumber = db.prepare('SELECT MAX(task_number) as max FROM tasks').get();
...
function updateTask(pattern, newStatus) {
  ...
  const validStatuses = ['ready', 'in-progress', 'pending', 'completed', 'blocked'];
  ...
  if (!task) {
    task = db.prepare('SELECT * FROM tasks WHERE text LIKE ? LIMIT 1').get(`%${pattern}%`);
  }
  ...
  blockedTasks.forEach(blockedTask => {
    ...
    if (newBlockers.length === 0) {
      ...
      if (blockedTask.status === 'blocked') {
        ...
        db.prepare('UPDATE tasks SET blocked_by = ? WHERE id = ?').run('[]', blockedTask.id);
        console.log(`→ Removed blocker from: #${blockedTask.id}`);
      }
    } else {
      // Still has other blockers - just update the array
      db.prepare('UPDATE tasks SET blocked_by = ? WHERE id = ?').run(JSON.stringify(newBlockers), blockedTask.id);
      console.log(`→ Updated blockers for: #${blockedTask.id} (remaining: ${newBlockers.join(', ')})`);
    }
  });
}
...
function countTasks() {
  ...
  console.log(`Ready: ${counts.ready || 0} | In Progress: ${counts.in_progress || 0} | Pending: ${counts.pending || 0} | Completed: ${counts.completed || 0} | Blocked: ${counts.blocked || 0}`);
}
