...
function gitCommit(cwd, message) {
  ...
  return commitResult.success || commitResult.stderr?.includes('nothing to commit');
}
...
function checkStatusSignals(workNotes) {
  ...
  for (let i = notes.length - 1; i >= 0; i--) {
    ...
    // Done signals
    if (content.includes('status:done') || content.includes('status:complete') || content.includes('status:completed')) {
...
function main() {
  ...
  try {
    ...
    if (inProgressTask) {
      ...
      // ======================================================================
      log('info', `Found in-progress task #${inProgressTask.task_number}: ${inProgressTask.text}`);

      const project = inProgressTask.project_id ? getProjectForTask(db, inProgressTask.id) : null;
      ...
      if (!project || !projectPath) {
        log('warn', `Task #${inProgressTask.task_number} has no valid project path - cannot process git operations`);
        ...
        if (signals.done) {
          ...
          appendWorkNote(db, inProgressTask.id, `Task marked completed (no project repo)`);
          log('info', `Marked task #${inProgressTask.task_number} as COMPLETED (no project)`);
          actionsTaken.push(`Completed: #${inProgressTask.task_number} (no project)`);
        } else if (signals.blocked) {
          db.prepare(`UPDATE tasks SET status = 'blocked', updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
            .run(inProgressTask.id);
          log('info', `Marked task #${inProgressTask.task_number} as BLOCKED: ${signals.blockedReason}`);
          actionsTaken.push(`Blocked: #${inProgressTask.task_number} - ${signals.blockedReason}`);
        } else {
          log('info', `Task #${inProgressTask.task_number} still in-progress (no project assigned)`);
        }
        ...
        const updatedAt = inProgressTask.updated_at
          ...
          : null;
        const minutesInProgress = updatedAt && !isNaN(updatedAt.valueOf())
          ? (Date.now() - updatedAt.getTime()) / 1000 / 60
          : null;

        if (minutesInProgress !== null && minutesInProgress >= STALE_THRESHOLD_MINUTES) {
          log('warn', `Stale task detected (#${inProgressTask.task_number}, ${Math.floor(minutesInProgress)}m). Sanitizing repo...`);
          if (isGitDirty(projectPath)) {
            execGit(projectPath, ['reset', '--hard']);
            execGit(projectPath, ['clean', '-fd']);
            appendWorkNote(db, inProgressTask.id, 'Stale rescue: repo reset + clean to recover from dirty state.');
        ...
        if (signals.done) {
          ...
          appendWorkNote(db, inProgressTask.id, `Task completed and pushed to remote`);
          actionsTaken.push(`Completed: #${inProgressTask.task_number} - ${inProgressTask.text}`);
          
        } else if (signals.blocked) {
          // BLOCKED: Save progress, mark blocked
          log('info', `Task #${inProgressTask.task_number} marked as BLOCKED: ${signals.blockedReason}`);
          ...
          appendWorkNote(db, inProgressTask.id, `Task blocked: ${signals.blockedReason}`);
          actionsTaken.push(`Blocked: #${inProgressTask.task_number} - ${signals.blockedReason}`);
          ...
          log('info', `Task #${inProgressTask.task_number} still in-progress (no changes detected)`);
        }
      }
      ...
      if (pendingTask) {
        ...
        log('info', `Found pending task #${pendingTask.task_number}: ${pendingTask.text} (${Math.floor(minutesPending)}m)`);

        if (minutesPending >= 5) {
          // 5+ minutes in pending - try to complete
          const project = pendingTask.project_id ? getProjectForTask(db, pendingTask.id) : null;
          const projectPath = getProjectPath(project);

          if (!project || !projectPath) {
            // No project - cannot complete, update timestamp to reset countdown
            log('warn', `Task #${pendingTask.task_number} has no project - skipping (will retry in 5m)`);
            db.prepare(`UPDATE tasks SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
              .run(pendingTask.id);
            actionsTaken.push(`Skipped: #${pendingTask.task_number} (no project)`);
          } else if (!fs.existsSync(projectPath)) {
            log('warn', `Project path does not exist: ${projectPath}`);
            db.prepare(`UPDATE tasks SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
              .run(pendingTask.id);
            ...
            if (signals.done && isDirty) {
              ...
              actionsTaken.push(`Completed: #${pendingTask.task_number} - ${pendingTask.text}`);
            } else if (signals.done && !isDirty) {
              // Mark complete without commit
              db.prepare(`UPDATE tasks SET status = 'completed', updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
                .run(pendingTask.id);
              appendWorkNote(db, pendingTask.id, 'Task completed (no changes)');
              log('info', `Completed task #${pendingTask.task_number} (no changes to commit)`);
              actionsTaken.push(`Completed: #${pendingTask.task_number} (no changes)`);
            } else if (!signals.done && isDirty) {
              ...
              log('info', `Task #${pendingTask.task_number} waiting (no signals, no changes)`);
              db.prepare(`UPDATE tasks SET updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
                .run(pendingTask.id);
            }
          }
          ...
          log('info', `Task #${pendingTask.task_number} waiting (${Math.floor(minutesPending)}m/5m)`);
        }
      }
      ...
      if (actionsTaken.length === 0) {
        ...
        if (readyTask) {
          log('info', `Picking up task #${readyTask.task_number}: ${readyTask.text}`);

          // Update status to in-progress
          db.prepare(`UPDATE tasks SET status = 'in-progress', updated_at = CURRENT_TIMESTAMP WHERE id = ?`)
            .run(readyTask.id);

          // Log start with planned approach
          const project = readyTask.project_id ? getProjectForTask(db, readyTask.id) : null;
          const projectInfo = project ? `Project: ${project.name}` : 'No project assigned';
          const approach = readyTask.notes ? readyTask.notes.substring(0, 200) : 'See task description';

          appendWorkNote(db, readyTask.id, `Started task. ${projectInfo}. Approach: ${approach}`);

          log('info', `Started task #${readyTask.task_number}`);
          actionsTaken.push(`Started: #${readyTask.task_number} - ${readyTask.text}`);
