...
const Database = require(path.join(WORKSPACE_ROOT, 'dashboard', 'node_modules', 'better-sqlite3'));
...
function appendWorkNoteIfMissing(taskId, marker, content, author = 'system') {
  const task = db.prepare('SELECT work_notes FROM tasks WHERE id = ?').get(taskId);
  if (!task) return;
  const existingNotes = task.work_notes ? JSON.parse(task.work_notes) : [];
  const hasMarker = existingNotes.some((note) =>
...
function getRepoChangeState(task) {
  if (!task?.project_id) return { hasRepo: false, hasChanges: false, behind: 0 };
  const project = getProjectById(task.project_id);
  if (!project) return { hasRepo: false, hasChanges: false, behind: 0 };
  const localPath = project.workspace_path || project.local_path;
  if (!localPath || !fs.existsSync(localPath)) return { hasRepo: false, hasChanges: false, behind: 0 };

  const statusResult = gitInProject(task.project_id, 'status', '--porcelain');
  const hasChanges = Boolean(statusResult.success && statusResult.output.trim());

  let behind = 0;
  const fetchResult = gitInProject(task.project_id, 'fetch', 'origin');
  if (fetchResult.success) {
    const branchResult = gitInProject(task.project_id, 'rev-parse', '--abbrev-ref', 'HEAD');
    const branch = branchResult.success ? branchResult.output.trim() : 'main';
    const behindResult = gitInProject(task.project_id, 'rev-list', '--count', `HEAD..origin/${branch}`);
...
function gitInProject(projectId, ...args) {
  ...
  if (!project) {
    return { success: false, output: '', error: `Project ${projectId} not found` };
  }
  
  const localPath = project.workspace_path || project.local_path;
  
  if (!localPath) {
    return { success: false, output: '', error: `Project ${projectId} has no local_path` };
  }
  
  if (!fs.existsSync(localPath)) {
    return { success: false, output: '', error: `Directory does not exist: ${localPath}` };
  }
...
// Helper: Commit task completion to project repo
function commitToProject(task) {
  if (!task.project_id) {
    return { skipped: true, reason: 'No project_id on task' };
  }
  
  const result = gitInProject(task.project_id, 'status', '--porcelain');
  ...
  const addResult = gitInProject(task.project_id, 'add', '.');
  if (!addResult.success) {
    return { success: false, error: `git add failed: ${addResult.error}` };
  }
  
  const commitResult = gitInProject(task.project_id, 'commit', `-m "Complete task #${task.task_number}: ${task.text.substring(0, 50)}..."`);
...
function completeWithSummary(taskId) {
  ...
  if (task.status !== 'in-progress') {
    console.error(`Task #${taskId} is not in-progress (current: ${task.status})`);
    return;
  }
  ...
  // Update task
  db.prepare('UPDATE tasks SET status = ?, work_notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
...
function appendWorkNote(taskId, content, author = 'agent') {
  ...
  db.prepare('UPDATE tasks SET work_notes = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
...
function updateBlockersForTask(completedTaskId) {
  const blockedTasks = db.prepare('SELECT * FROM tasks WHERE blocked_by LIKE ?').all(`%${completedTaskId}%`);
  ...
  blockedTasks.forEach(blockedTask => {
    ...
    if (newBlockers.length === 0) {
      if (blockedTask.status === 'blocked') {
        db.prepare('UPDATE tasks SET blocked_by = ?, status = ? WHERE id = ?').run('[]', 'ready', blockedTask.id);
        console.log(`→ Unblocked and moved to ready: #${blockedTask.task_number}`);
      }
    }
  });
}
...
console.log(`Backlog: ${counts.backlog || 0} | Ready: ${counts.ready || 0} | In Progress: ${counts.in_progress || 0} | Blocked: ${counts.blocked || 0} | Completed: ${counts.completed || 0}`);
if (PROJECT_FILTER && PROJECT_FILTER !== 'all') {
...
if (currentTask) {
  ...
  console.log(`Priority: ${currentTask.priority || 'none'} | Time: ${minutesInProgress} min`);
  if (projectRoot) {
  ...
  if (currentTask.notes) {
    console.log(`Notes: ${currentTask.notes.substring(0, 200)}${currentTask.notes.length > 200 ? '...' : ''}`);
  }
  ...
  if (minutesInProgress >= STUCK_THRESHOLD_MINUTES) {
    ...
    console.log(`This task has been in progress for ${minutesInProgress} minutes.`);
    ...
    if (!hasProgressDetails) {
      ...
      if (repoState.hasRepo && (repoState.hasChanges || repoState.behind > 0)) {
        ...
        db.prepare('UPDATE tasks SET updated_at = CURRENT_TIMESTAMP WHERE id = ?')
          .run(currentTask.id);
      }
    }
    ...
    if (!hasProgressDetails) {
      ...
      console.log('2. Run: node scripts/recurring-worker.js --complete-with-summary <task-id>');
    }
    ...
    if (hasProgressDetails) {
      console.log('   - Run: node scripts/recurring-worker.js --complete-with-summary ' + currentTask.id);
      ...
      console.log('   - THEN: Run: node scripts/recurring-worker.js --complete-with-summary ' + currentTask.id);
    }
    console.log('3. If not complete: continue working on it (stay in-progress)');
    console.log('');
    console.log('→ Do NOT pick up new ready tasks until this one is resolved.');
  } else {
    console.log('');
    console.log(`→ Complete this task before picking up a new one (${STUCK_THRESHOLD_MINUTES - minutesInProgress} min until stuck detection).`);
    if (!hasProgressDetails) {
  ...
  if (process.argv.includes('--complete-with-summary')) {
    const taskId = parseInt(process.argv[process.argv.indexOf('--complete-with-summary') + 1]);
    if (taskId) {
...
if (!nextTask) {
  ...
  for (const task of readyTasks.slice(0, 5)) {
    ...
    console.log(`  Blocked by: ${blockers.map(b => `#${b.task_number}`).join(', ')}`);
  }
...
const waitingOnThis = db.prepare(`
  SELECT task_number, text FROM tasks
  WHERE status = 'ready'
...
console.log('   For tasks returning from stuck state, check if complete first.');
console.log('\n=== AGENT PROMPT (STRICT) ===');
...
console.log('3) Update work_notes throughout the process (progress + decisions).');
console.log('4) If blocked or unclear, set status to BLOCKED and log why.');
console.log('5) If you commit, run git from the Project Root (CWD), not the workspace root.');
console.log('PROCESS:');
console.log('- Start with a short plan in work_notes: "Started: <short plan>"');
console.log('- Make changes in small steps; log each change: "Progress: <what changed>"');
...
console.log('- Use Clawddocs for current docs.');
console.log('- Use notebooklm-skill (ID: 132520db-e751-4ea2-9512-d2b9418c8ecb).');
...
console.log('- Confirm git status is clean or changes are committed from Project Root.');
