...
const DB_PATH =
  process.env.TASKS_DB_PATH || path.join(WORKSPACE_ROOT, "data", "tasks.db");
...
function provisionWorkspace(project) {
  ...
  if (exists) {
    ...
    if (!workspace_path && !DRY_RUN) {
      updateDb(
        "UPDATE projects SET workspace_path = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?",
        [targetPath, id],
      );
...
function main() {
  ...
  log(
    `Summary: ${provisioned} created/updated, ${skipped} skipped, ${errors} errors`,
  );
